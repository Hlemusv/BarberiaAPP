...
async function mostrarServicios() {
  try {
    ...
    // const { servicios } = db;  //ya no se necesita porque se remplaza para el for each
    // Generar el HTML
    db.forEach((servicio) => {
...
function mostrarResumen() {
  ...
  if (Object.values(cita).includes("")) {
    ...
    noServicios.textContent =
      "Ingresa porfa de forma correcta tu nombre, el servicio que quieres y la fecha en que lo quieres";
  ...
  headingCita.textContent = "El resumen de tu cita es:";
...
function nombreCita() {
  ...
  nombreInput.addEventListener("input", (e) => {
    ...
    if (nombreTexto === "" || nombreTexto.length < 3) {
      mostrarAlerta("Revisa bien como te llamas", "error");
    } else {
